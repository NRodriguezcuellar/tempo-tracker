...
async function startTrackingSession(
  ...
  try {
    ...
    log(
      `Started tracking session ${session.id} in ${directory} on branch ${branch}`
    );
  } catch (error: any) {
...
async function stopTrackingSession(sessionId: string): Promise<void> {
  try {
    ...
    // Add to activity log in main config
    const config = await getConfig();
...
function cleanup(): void {
  log("Cleaning up daemon resources");
...
initDaemon().catch((error) => {
  log(`Fatal error initializing daemon: ${error.message}`);
  process.exit(1);
});
